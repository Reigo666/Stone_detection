...
from .my_test_mixins import BBoxTestMixin, MaskTestMixin
from .mask_heads import FCNMaskScoreHead


@HEADS.register_module()
class MyCascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(MyCascadeRoIHead, self).__init__(
    ...
    def init_mask_head(self, mask_roi_extractor, mask_head):
        ...
        self.mask_iou_head = FCNMaskScoreHead()
    ...
    def _mask_iou_forward_train(self,
                                sampling_results,
                                gt_masks,
                                rcnn_train_cfg,
                                mask_results):
        mask_targets = self.mask_head[-1].get_targets(
            sampling_results, gt_masks, rcnn_train_cfg)
        pos_labels = torch.cat([res.pos_gt_labels for res in sampling_results])
        pos_mask_pred = mask_results['mask_pred'][range(mask_results['mask_pred'].size(0)), pos_labels]
        mask_iou_pred = self.mask_iou_head(pos_mask_pred, mask_results['mask_feats'])
        pos_mask_iou_pred = mask_iou_pred[range(mask_iou_pred.size(0)), pos_labels]
        mask_iou_targets = self.mask_iou_head.get_targets(sampling_results, gt_masks, pos_mask_pred, mask_targets, self.train_cfg)
        mask_iou_loss = self.mask_iou_head.loss(pos_mask_iou_pred, mask_iou_targets)
        return mask_iou_loss
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if i == self.num_stages - 1:
                mask_iou_loss = self._mask_iou_forward_train(sampling_results, gt_masks, rcnn_train_cfg, mask_results)
                losses.update(mask_iou_loss)
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if self.with_mask:
            class_nums = self.bbox_head[-1].num_classes
            batch_size = len(det_bboxes)
            for i in range(batch_size):
                for j in range(class_nums):
                    # ious = np.array(list(map(self.box2iou, segm_results[i][j], bbox_results[i][j])))
                    ms_bbox_result['ensemble'][i][j][:, -1] = np.maximum(ms_bbox_result['ensemble'][i][j][:, -1], mask_scores[i][j])
            results = list(
